...
//> Functions interpreter-constructor
  Interpreter() {
    globals.define("clock", new PseuciCallable() {
      @Override
...
//> Inheritance interpret-superclass
    ...
    if (stmt.superclass != null) {
      ...
      if (!(superclass instanceof PseuciClass)) {
...
//> interpret-methods
    ...
    Map<String, PseuciFunction> methods = new HashMap<>();
    for (Stmt.Function method : stmt.methods) {
/* Classes interpret-methods < Classes interpreter-method-initializer
      PseuciFunction function = new PseuciFunction(method, environment);
*/
...
/* Classes interpret-methods < Inheritance interpreter-construct-class
    PseuciClass klass = new PseuciClass(stmt.name.lexeme, methods);
*/
//> Inheritance interpreter-construct-class
    PseuciClass klass = new PseuciClass(stmt.name.lexeme,
        (PseuciClass)superclass, methods);
...
/* Functions visit-function < Functions visit-closure
    PseuciFunction function = new PseuciFunction(stmt);
*/
/* Functions visit-closure < Classes construct-function
    PseuciFunction function = new PseuciFunction(stmt, environment);
*/
//> Classes construct-function
    PseuciFunction function = new PseuciFunction(stmt, environment, false);
//< Classes construct-function
...
//> check-is-callable
    if (!(callee instanceof PseuciCallable)) {
      throw new RuntimeError(expr.paren,
          "Can only call functions and classes.");
    }

//< check-is-callable
    PseuciCallable function = (PseuciCallable)callee;
//> check-arity
...
//> Classes interpreter-visit-get
  ...
  public Object visitGetExpr(Expr.Get expr) {
    ...
    if (object instanceof PseuciInstance) {
      return ((PseuciInstance) object).get(expr.name);
    }
...
//> Classes interpreter-visit-set
  ...
  public Object visitSetExpr(Expr.Set expr) {
    ...
    if (!(object instanceof PseuciInstance)) { // [order]
      throw new RuntimeError(expr.name, "Only instances have fields.");
    }

    Object value = evaluate(expr.value);
    ((PseuciInstance)object).set(expr.name, value);
    return value;
  }
//< Classes interpreter-visit-set
...
//> super-find-method
    ...
    PseuciFunction method = superclass.findMethod(expr.method.lexeme);
//> super-no-method
